...
def main(args):
    ...
    if not args.eval:
        if not args.val:
            train_dataset = ImagerLoaderVit(args.source_path, args.train_file, args.json_path, args.gt_path, stride=args.train_stride, single=args.single)
            cnt1 = train_dataset.cnt1
            cnt2 = train_dataset.cnt2
        ...
        if(not args.flip_test):
            test_dataset = TestImagerLoaderVit("/Ego4D_LookAtMe/internvitfeatest", single=args.single)
        else:
            test_dataset = TestImagerLoaderVit("/Ego4D_LookAtMe/internvitfeatestflip", single=args.single)
